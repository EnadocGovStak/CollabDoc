...
export const createStyledSfdtFromText = (textContent, options = {}) => {
  ...
  if (options.title && firstContentIndex === -1) {
    sfdtDocument.sections[0].blocks.unshift(createParagraph(options.title, {
      styleName: 'Title',
      afterSpacing: 14,
      textAlignment: 'Center'
    }, {
...
export const normalizeSfdtContent = (content, options = {}) => {
...
export const extractTextFromSfdtContent = (content) => {
  if (!content) return '';
...
export const extractMergeFieldsFromSfdtContent = (content) => extractMergeFieldsFromText(extractTextFromSfdtContent(content));
...
const replaceMergeFieldsInText = (text, mergeData = {}) => {
  ...
  Object.entries(mergeData || {}).forEach(([key, value]) => {
    ...
    result = result.replace(pattern, value !== null && value !== undefined ? String(value) : '');
  });
...
export const mergeSfdtContent = (content, mergeData = {}) => {
