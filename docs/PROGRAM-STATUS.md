...
| P1 | D4 - Template Generation And Authoring UX | Started | First guided Generate from Template pass plus guided `/templates/new` authoring banner/checklist, UI/API governed-save readiness enforcement, and unmanaged-field review/promotion around the current editor | Richer field validation rules still need careful follow-up. |
...
| Template editor | Improved | Existing invoice template metadata and editor surface load in browser smoke; the editor now shows save-readiness notes, blocks governed saves until notes are resolved or saved as drafts, sends explicit draft/governed save mode to the API, shows managed field analysis with Insert/Copy controls, and uses a review form before adding unmanaged fields to the Field Library. `/templates/new` has a Guided creation banner/checklist around the stable editor surface. The Professional Invoice editor route was verified with 18 managed fields and no stuck loading overlay. |
...
| Managed field library | Started | JSON-backed library exposes 33 reusable fields through API routes, enriches template detail/list/search/save responses, reports unknown placeholders through field analysis, blocks incomplete governed saves at the API, and now has a first UI for listing/filtering/editing fields. |
...
Sprint 02 should continue with debt-prioritized design work: finish D4 richer field validation rules after the governed-save enforcement slice, then approach D5 editor chrome only with explicit Syncfusion route-change regression checks in place.
...
- May 26 SFlow identity readiness validation: public OIDC discovery metadata was verified at `/identity/.well-known/openid-configuration`; backend JWT middleware loaded with the SFlow issuer and JWKS defaults; auth-disabled collaboration smoke accepted local demo identity; `AUTH_REQUIRED=true` collaboration smoke rejected a missing bearer token with `401`; `npm --prefix frontend run build` compiled successfully after frontend bearer-token forwarding support was added. Interim SFlow integration reuses `govstack.workflow` for CollabDoc demo tokens, with Azure SFlow client `collabdoc-ui-spa` and `AUTH_ALLOWED_CLIENT_IDS=collabdoc-ui-spa` available to reduce audience-confusion risk until a first-class CollabDoc scope/audience is added. Azure SFlow Identity was updated to `sflow-identity:collabdoc-pkce-fix-20260526a`, `collabdoc-ui-spa` was repaired through the client-management API, and `/identity/connect/authorize` now redirects to login for both `http://localhost:3001/auth/callback` and `https://collabdocweb-fresh.azurewebsites.net/auth/callback`.