...
| Sprint 02- Template Foundation | In Progress | Managed reusable template fields, reliable guided generation, field migration review, save-readiness notes, and first Evia Prism shell/list/account rollout | Deeper validation enforcement, richer field validation rules, Template Editor authoring polish, and editor-safe chrome polish are still pending. |
...
| P1 | D4 - Template Generation And Authoring UX | Started | First guided Generate from Template pass plus guided `/templates/new` authoring banner/checklist, save-readiness notes, and unmanaged-field review/promotion around the current editor | Deeper authoring validation enforcement and richer field validation rules still need careful follow-up. |
...
| Template editor | Improved | Existing invoice template metadata and editor surface load in browser smoke; the editor now shows save-readiness notes, managed field analysis with Insert/Copy controls, and a review form before adding unmanaged fields to the Field Library. `/templates/new` has a Guided creation banner/checklist around the stable editor surface. The Professional Invoice editor route was verified with 18 managed fields and no stuck loading overlay. |
...
- Template schema is inconsistent across stored templates; Sprint 02 now has a managed field-library API, analysis bridge, editor-side unmanaged-field migration review, save-readiness notes, and save-time metadata enrichment, but deeper validation enforcement is still needed.
...
Sprint 02 should continue with debt-prioritized design work: finish D4 authoring validation enforcement and focused regression coverage after the first migration review/readiness slice, then approach D5 editor chrome only with explicit Syncfusion route-change regression checks in place.
