...
| Sprint 02- Template Foundation | In Progress | Managed reusable template fields, reliable guided generation, first field migration action, and first Evia Prism shell/list/account rollout | Deeper validation enforcement, richer field-detail editing during migration, Template Editor authoring polish, and editor-safe chrome polish are still pending. |
...
| P1 | D4 - Template Generation And Authoring UX | Started | First guided Generate from Template pass plus guided `/templates/new` authoring banner/checklist and unmanaged-field promotion action around the current editor | Deeper authoring validation and richer field-detail editing still need careful follow-up. |
...
| Template editor | Improved | Existing invoice template metadata and editor surface load in browser smoke; the editor now shows managed field analysis with Insert/Copy controls plus Add to Field Library actions for unmanaged fields, and `/templates/new` has a Guided creation banner/checklist around the stable editor surface. The Professional Invoice editor route was verified with 18 managed fields and no stuck loading overlay. |
...
- Template schema is inconsistent across stored templates; Sprint 02 now has a managed field-library API, analysis bridge, editor-side unmanaged-field promotion action, and save-time metadata enrichment, but deeper validation and richer migration review are still needed.
...
Sprint 02 should continue with debt-prioritized design work: finish D4 authoring validation and migration-detail polish after the first unmanaged-field promotion action, then approach D5 editor chrome only with explicit Syncfusion route-change regression checks in place.
